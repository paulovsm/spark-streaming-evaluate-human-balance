...
```
docker exec -it spark-streaming-evaluate-human-balance_redis_1 redis-cli
```
...
```
docker exec -it spark-streaming-evaluate-human-balance_kafka_1 kafka-console-consumer --bootstrap-server localhost:9092 --topic redis-server
```
...
```
docker logs spark-streaming-evaluate-human-balance_spark_1 >& spark/logs/spark-master.log

docker logs spark-streaming-evaluate-human-balance_spark_1 >& spark/logs/spark-master.log >& spark/logs/spark-worker.log
```
...
```
docker logs -f spark-streaming-evaluate-human-balance_stedi_1
```
